...
class PickAndPlace:
	...
	def __init__(self):
		...
		#set color
		self.setColor(self.table1_id,0.8,0,0,1.0)
		self.setColor(self.table2_id,0.8,0,0,1.0)
		self.setColor(self.table3_id,0.8,0,0,1.0)
		self.setColor(self.table4_id,0.8,0,0,1.0)
		...
		self.add_target(target1_pose,self.reference_frame,x,y,0,0,0,1)
		self.scene.add_box(target1_id,target1_pose,target1_size)
		self.add_target(target2_pose,self.reference_frame,x,y,0,0,0,1)
		self.scene.add_box(target2_id,target2_pose,target2_size)
		self.add_target(target3_pose,self.reference_frame,x,y,0,0,0,1)
		...
		for i in range(tar_num):
			...
			if i==1:
				...
				end_pose=target2_pose
				target_id=target2_id
				target_size=target2_size
			elif i==3:
				end_pose=target3_pose
				target_id=target3_id
				target_size=target3_size
			elif i==4:
				end_pose=target4_pose
				target_id=target4_id
				target_size=target4_size
			#pick up
			if cartesian:
				plan_1, end_joint_state_1,signal_1=self.cts(start_pose,target_pose,maxtries)
				if signal_1:
					...
					self.gripperCtrl(0)
					rospy.sleep(2)
					self.scene.attach_box(self.arm_end_effector_link,target_id,g_p,target_size)
