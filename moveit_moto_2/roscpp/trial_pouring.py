...
class PickAndPlace:
	...
	def __init__(self):
		...
		rospy.sleep(1)
		self.add_target(f_target_pose,self.f_target_size,self.reference_frame,-msg2.position.y-0.257,-msg2.position.x+0.81-0.12-l_gripper,0,0,0,1)
		self.scene.add_box(self.f_target_id,f_target_pose,self.f_target_size)
		...
		for i in range(0,tar_num):
			...
			pour_angle=[0.0]
			tip_angle=[50.0,60.0,70.0,80.0,90.0]
			r_angle_delta=5.0
			#~ r_angle=[5.0,10.0,15.0,20.0,25.0,30.0,35.0,40.0,45.0,50.0,55.0,60.0,65.0,70.0,75.0,80.0,85.0,90.0]
			...
			for m in range(len(pour_angle)):
				dfile=open("moyashi_tip_1","a")
				for j in range(len(tip_angle)):
					for n in range(6):
						pp_ps_sgn=self.pp_ps(f_target_pose,pour_angle[m],r_pour,h_pour)
						if pp_ps_sgn==0: 
							...
							continue
						initial_pose=self.arm.get_current_pose(self.arm_end_effector_link)
						pour_signal=self.pour_rotate(initial_pose,pour_angle[m],tip_angle[j],r_bottle,maxtries)
						if pour_signal !=1:
						...
						#~ area_ratio= rospy.wait_for_message("/ratio_pepper",Float64)

						dfile.write("%f %f\r\n" % (tip_angle[j],area_ratio.data))
						
						rospy.loginfo("pouring degree : "+str(tip_angle[j])+", times: "+str(n))
