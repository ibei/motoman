...
def build_traj(start, end, duration):
  """Build a trajectory with the given start and end position
  Arguments:
    start - trajectory starting position
    end - trajectory ending position
    duration - trajectory duration
  Return:
    JointTrajectory() actually only returns joint configurations at the specified endpoint
  """
  ...
  # create joint trajectory point instance (start point)
  ...
  # create joint trajectory point instance (end point)
...
def wait_for_subs(pub, num_subs, min_time, timeout):
  ...
  print("get sub num " + str(pub.get_num_connections()))
...
def move_to_joint(end_pos, duration):
  ...
  pub = rospy.Publisher('joint_path_command', JointTrajectory, queue_size=1) # works with rviz and real robot
  pub_sim = rospy.Publisher('/sia10f_controller/command', JointTrajectory, queue_size=1) # works with gazebo
